...
def compute_team_coefficients(df, window=10, min_games=3):
    ...
    # 2. 安全地构建最近 window 场主/客场数据
    records = []
    for team, group in all_matches.groupby('team'):
        for venue in ['home', 'away']:
            venue_games = group[group['venue'] == venue].sort_values('date')
            recent = venue_games.tail(window)
            if len(recent) > 0:
                records.append(recent)
    if not records:
        # 没有数据时返回空表，防止后续报错
        return pd.DataFrame(), {'Lg_ATT_H': 1, 'Lg_ATT_A': 1, 'Lg_DEF_H': 1, 'Lg_DEF_A': 1}

    recent_by_venue = pd.concat(records, ignore_index=True)
    ...
    if len(valid_home) == 0 or len(valid_away) == 0:
        league_avg = {'Lg_ATT_H': 1, 'Lg_ATT_A': 1, 'Lg_DEF_H': 1, 'Lg_DEF_A': 1}
    else:
        league_avg = {
